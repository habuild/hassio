...
CFG_PATH="${1:-SBFspot.cfg}"
CFG_PATH_UPLOAD="${2:-SBFspotUpload.cfg}"
#CONFIG_PATH=/data/options.json #not needed with bashio
...
CONFIG_LATITUDE="$(bashio::config "${Latitude:-0}")"
CONFIG_LONGITUDE="$(bashio::config "${Longitude:-0}")"
...
#  CONFIG_MQTT_PUBLISHERARGS="$(bashio::config "${MQTT_PublisherArgs:-0}")"
CONFIG_MQTT_DATA="$(bashio::config "${MQTT_Data:-Timestamp,SunRise,SunSet,InvSerial,InvName,InvTime,InvStatus,InvSwVer,InvClass,InvType,InvTemperature,OperTm,FeedTm,EToday,ETotal,GridFreq,PACTot,PAC1,UAC1,IAC1,PDCTot,UDC1,UDC2,IDC1,IDC2,PDC1,PDC2,BTSignal}")"
#  CONFIG_LOGDIR="$(bashio::config "${LogDir:-0}")"
CONFIG_LOGLEVEL="$(bashio::config "${LogLevel:-0}")"
